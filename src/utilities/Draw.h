...
namespace Draw
{
	...
	void Render3DCircleOneteap(Vector vecPosition, Color colColor, int nPointCount, float flRadius, bool bFade, float flRotationStart, float flFadeStart, float flFadeLength);

	// Values
	/* directx init state */
	inline bool	m_bInitialized = false;
	/* saved data to draw */
	inline std::deque<DrawObject_t> m_vecDrawData = { };
	/* thread-safe data to draw */
	inline std::deque<DrawObject_t> m_vecSafeDrawData = { };
	/* thread-safe render mutex */
	inline std::shared_mutex m_drawMutex = { };
}