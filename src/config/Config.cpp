...
bool Config::Setup(std::string_view szDefaultFileName)
{
	...
	if (!Save(szDefaultFileName, true))
		return false;
...
bool Config::Save(std::string_view szFileName, bool bInit)
{
	...
	if (bInit)
	{
		...
		{
			std::cout << X("Skipping creation of default config, file already exists: ") << szFile << std::endl;
			return true;
		}
	}
	...
	try
	{
		for (auto& variable : m_vecVariables)
		{
			...
			switch (variable.uTypeHash)
			{
				...
				case FNV1A::HashConst("std::vector<KeyBind_t>"):
				{
					const auto vecKeyBinds = variable.Get<std::vector<KeyBind_t>>();

					nlohmann::json sub = { };

                    for (const auto& keybindValue : vecKeyBinds)  
                       sub.push_back(const_cast<KeyBind_t&>(keybindValue).PackIntoBits());
...
bool Config::Load(std::string_view szFileName)
{
	...
	try
	{
		for (const auto& variable : config)
		{
			...
			if (nIndex == CONFIG_INVALID_VARIABLE)
				continue;
			...
			switch (variable[X("type-id")].get<FNV1A_t>())
			{
				...
				{
					const nlohmann::json vector = nlohmann::json::parse(variable[X("value")].get<std::string>());
					auto& vecKeyBinds = entry.Get<std::vector<KeyBind_t>>();

					for (std::size_t i = 0U; i < vector.size(); i++)
					{
						if (i < vecKeyBinds.size())
							vecKeyBinds.at(i) = KeyBind_t::UnpackFromBits(vector.at(i).get<unsigned>());
					}
...
std::size_t Config::GetVariableIndex(const FNV1A_t uNameHash)
{
	...
	return CONFIG_INVALID_VARIABLE;
}
